...
const MultiSelect: React.FC<MultiSelectProps> = ({ options, selectedOptions, onChange }) => {
  ...
  return (
    <div className="relative mb-4">
      ...
      {isOpen && (
        <div className="absolute border mt-1 bg-white dark:bg-gray-700 w-full z-10 rounded-md shadow-lg">
          {options.map(option => (
            <div
              key={option.value}
              className={`p-2 cursor-pointer ${selectedOptions.includes(option.value) ? 'bg-blue-500 text-white' : 'text-gray-900 dark:text-gray-300'}`}
